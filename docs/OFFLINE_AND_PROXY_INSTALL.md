...
Версия в примерах: подставьте свою (см. `pyproject.toml`) вместо `**0.4.0`**.
...
- Файл `**sdocs-mcp-ui-0.4.0.tar`** (`docker save` готового образа).
...
```bash
docker save sdocs-mcp-ui:0.4.0 -o sdocs-mcp-ui-0.4.0.tar
```
