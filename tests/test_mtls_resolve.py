...
def test_mtls_none_when_env_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("STACK_MCP_MTLS_CERT_FILE", raising=False)
    monkeypatch.delenv("STACK_MCP_MTLS_KEY_FILE", raising=False)
    monkeypatch.delenv("STACK_MCP_MTLS_ROOT_CA_FILE", raising=False)
...
def test_mtls_incomplete_returns_none(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    monkeypatch.setenv("STACK_MCP_MTLS_CERT_FILE", "/x/crt")
    monkeypatch.delenv("STACK_MCP_MTLS_KEY_FILE", raising=False)
    monkeypatch.delenv("STACK_MCP_MTLS_ROOT_CA_FILE", raising=False)
...
def test_mtls_ok(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("STACK_MCP_MTLS_CERT_FILE", str(cert))
    monkeypatch.setenv("STACK_MCP_MTLS_KEY_FILE", str(key))
    monkeypatch.setenv("STACK_MCP_MTLS_ROOT_CA_FILE", str(ca))
    monkeypatch.setenv("STACK_MCP_MTLS_KEY_PASSWORD", "secret")
