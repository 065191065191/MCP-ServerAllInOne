...
- MCP tool **`sdocs_alerting_status`** (нет tools `alerting_*` — только этот статус + UI `/api/alerts/*`).

### Исправлено

- **PostgreSQL:** `ILIKE` без `%` в строке запроса (`||` конкатенация) — устраняет `%p` на любой версии psycopg.
- **`config_diagnose`:** секция `modules.alerting` не считается «неизвестной».
