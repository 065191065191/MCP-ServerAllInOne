...
После локальной сборки в каталоге может лежать **`stack-mcp-ui-0.2.7.tar`** (`docker save`). Файл в `.gitignore` из‑за размера. Импорт на целевой машине:

```bash
docker load -i stack-mcp-ui-0.2.7.tar
