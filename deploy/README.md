...
После локальной сборки в каталоге может лежать **`sdocs-mcp-ui-0.3.2.tar`** (`docker save`). Файл в `.gitignore` из‑за размера. Импорт на целевой машине:
