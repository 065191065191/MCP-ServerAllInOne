...
После локальной сборки в каталоге может лежать **`sdocs-mcp-ui-0.6.2.tar`** (`docker save`). Файл в `.gitignore` из‑за размера. Импорт на целевой машине:
...
docker run --rm -p 127.0.0.1:8765:8765 \
  ...
  -v /path/on/host/config.yaml:/etc/sdocs-mcp/config.yaml:ro \
  sdocs-mcp-ui:0.6.2 sdocs-mcp
```

Пересобрать архив из корня репозитория:

```bash
docker build -f deploy/Dockerfile -t sdocs-mcp-ui:0.6.2 .
docker save sdocs-mcp-ui:0.6.2 -o deploy/sdocs-mcp-ui-0.6.2.tar
```
